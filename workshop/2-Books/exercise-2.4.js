...
class BookList {
  ...
  // to point to it.
  startReading = (title) => {
    let readingBook = this.BookList.find(
      (readingBook) => readingBook.title === title
    );
    this.currentlyReading = readingBook;
  };
  // When we finish reading a book, we should set `currentlyReading` back to `null`,
  // and set `lastRead` to the book we just finished.
  finishReading = (title) => {
    let book = this.BookList.find((book) => book.title === title);
    this.currentlyReading = null;
    this.lastRead = book;
    book.isRead = true;
  };
